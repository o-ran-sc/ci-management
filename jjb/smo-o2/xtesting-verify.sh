...
sudo useradd -s /bin/bash -d /opt/stack -m stack -p devstack
sudo chmod +x /opt/stack
echo "stack ALL=(ALL) NOPASSWD: ALL" | sudo tee /etc/sudoers.d/stack
sudo chown -R stack:jenkins /opt/stack
...
yes Y | sudo apt-get remove virtualenv  
yes Y | sudo apt-get purge python-virtualenv 
yes Y | sudo apt remove python3-virtualenv
...
grep "stack.s completed" /opt/stack/logs/stack.sh.log.summary
if [ !$? ]; then
  echo "Continue"
else
...
source /opt/jenkins/devstack/openrc nfv_user nfv
...
openstack vim register --config-file vim_config.yaml vim-kubernetes --is-default --debug
...
sudo mysql -uroot -pdevstack -h127.0.0.1 -Dtacker -e "update vims set extra=json_object(
         'helm_info', '{"masternode_ip": ["127.0.0.1"], "masternode_username": "stack", "masternode_password": "devstack"}')
         where id='$vim'";

#Execute script 'packageTest.sh' for package creation and uploading
cd /opt/stack/tacker/tacker/tests/xtesting/api-tests/SOL005/CNFPrecondition
...
cd /opt/stack/tacker/tacker/tests/xtesting/
. xtesting-py3/bin/activate
